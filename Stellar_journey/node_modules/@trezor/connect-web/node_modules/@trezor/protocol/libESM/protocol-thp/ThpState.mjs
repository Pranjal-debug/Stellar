...
export class ThpState {
  ...
  _sessionIdCounter = 0;
  _piggybackAckAvailable = false;
  _piggybackAckEnabled = false;
  _recentMessage = Buffer.alloc(0);
  get recentMessage() {
  ...
  sync(type, messageType) {
    const updateSyncBit = !['ThpAck', 'ThpCreateChannelRequest', 'ThpCreateChannelResponse'].includes(messageType);
    if (updateSyncBit) {
  ...
  deserialize(json) {
    ...
    this._piggybackAckEnabled = typeof json.piggybackAckEnabled === 'boolean' ? json.piggybackAckEnabled : false;
  }
